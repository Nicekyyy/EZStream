...
pub fn run() {
  let api_child = Arc::new(Mutex::new(None));
      ...
      #[cfg(not(debug_assertions))]
      {
        ...
        tauri::async_runtime::spawn(async move {
          ...
          if !storage_dir_path.exists() {
            std::fs::create_dir_all(&storage_dir_path).expect("Failed to create storage directory");
          }
          ...
          let api_script_path = clean_path(&api_dist_dir.join("apps/api/dist/apps/api/src/main.js"));
          let prisma_cli_path = clean_path(&api_dist_dir.join("node_modules/prisma/build/index.js"));
          let schema_path = clean_path(&api_dist_dir.join("packages/db/prisma/schema.prisma"));
          ...
          let migration_command = match app_handle.shell().sidecar("node") {
            ...
            Err(e) => {
              log(&format!("Failed to initialize node sidecar for migration: {:?}", e));
              return;
            }
          };
              ...
              while let Some(event) = mig_rx.recv().await {
                match event {
                  CommandEvent::Stdout(line) => {
                    log(&format!("Migration stdout: {}", String::from_utf8_lossy(&line).trim()));
                  }
                  CommandEvent::Stderr(line) => {
                    log(&format!("Migration stderr: {}", String::from_utf8_lossy(&line).trim()));
                  }
                  _ => {}
                }
              }
          ...
          if !db_existed {
            ...
            let seed_script_path = clean_path(&api_dist_dir.join("packages/db/dist/prisma/seed.js"));
            let seed_command = match app_handle.shell().sidecar("node") {
              Ok(cmd) => cmd
                .arg(&seed_script_path)
                .env("DATABASE_URL", &db_url),
              Err(e) => {
                log(&format!("Failed to initialize node sidecar for seed: {:?}", e));
                return;
              }
            };
            ...
            match seed_command.spawn() {
              Ok((mut seed_rx, _seed_child)) => {
                while let Some(event) = seed_rx.recv().await {
                  match event {
                    CommandEvent::Stdout(line) => {
                      log(&format!("Seed stdout: {}", String::from_utf8_lossy(&line).trim()));
                    }
                    CommandEvent::Stderr(line) => {
                      log(&format!("Seed stderr: {}", String::from_utf8_lossy(&line).trim()));
                    }
                    _ => {}
                  }
                }
          ...
          let api_command = match app_handle.shell().sidecar("node") {
            ...
            Err(e) => {
              log(&format!("Failed to initialize node sidecar for NestJS API: {:?}", e));
              return;
            }
          };
            ...
            Ok((mut api_rx, child)) => {
              ...
              *api_child_setup.lock().unwrap() = Some(child);
              ...
              while let Some(event) = api_rx.recv().await {
                match event {
                  CommandEvent::Stdout(line) => {
                    log(&format!("API stdout: {}", String::from_utf8_lossy(&line).trim()));
                  }
                  CommandEvent::Stderr(line) => {
                    log(&format!("API stderr: {}", String::from_utf8_lossy(&line).trim()));
                  }
                  _ => {}
                }
              }
