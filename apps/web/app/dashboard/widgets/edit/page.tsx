...
function WidgetDetailContent() {
  ...
  async function saveViewerCountSettings(nextConfig: Record<string, any>) {
    await updateWidget({ config: { ...widgetConfig, ...nextConfig } }, "บันทึกการตั้งค่า Viewer Count แล้ว");
  }
    ...
    <DashboardShell title="จัดการ Widget">
      ...
      <div className="flex flex-col-reverse gap-4 xl:grid xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-4">
          ...
          {isViewerCountWidget ? (
            <ViewerCountWidgetSettings busy={busy} config={widgetConfig} onSave={saveViewerCountSettings} />
          ) : null}
...
function ViewerCountWidgetSettings({
  ...
  onSave: (nextConfig: Record<string, any>) => Promise<void>;
}) {
  const [label, setLabel] = useState(config.label ?? "");
  const [platforms, setPlatforms] = useState(config.platforms ?? "all");
  const [showPlatforms, setShowPlatforms] = useState(config.showPlatforms !== false);
  const [showBackground, setShowBackground] = useState(config.showBackground !== false);
  ...
  return (
    <ResourceCard>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Button disabled={busy} onClick={() => void onSave({ label, platforms, showPlatforms, showBackground })} type="button">บันทึก</Button>
      </div>

      <div className="space-y-4">
        <ToggleField disabled={busy} label="แสดงพื้นหลังสีดำโปร่งแสง" checked={showBackground} onChange={setShowBackground} />
        
        <Field label="ช่องทางผู้ชมที่ต้องการแสดง">
          <Select disabled={busy} value={platforms} onChange={(event) => setPlatforms(event.target.value)}>
...
export default function WidgetDetailPage() {
  return (
    <Suspense fallback={<div className="p-4 text-xs text-white">Loading widget detail...</div>}>
      <WidgetDetailContent />
    </Suspense>
  );
}
